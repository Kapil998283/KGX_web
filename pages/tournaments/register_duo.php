...
try {
    ...
    // Verify it's a duo tournament
    if ($tournament['mode'] !== 'Duo') {
        redirect('details.php?id=' . $tournament['id'], 'This is not a duo tournament.');
    }
    ...
    if (!$team) {
        redirect('../../pages/teams/create_team.php?redirect=tournament&id=' . $tournament['id'], 
                'You must be a team captain to register for duo tournaments.');
    }
    ...
    if (empty($team_members)) {
        $error_message = "You need at least one team member to register for duo mode.";
    } elseif (count($available_members) < 1) {
        $error_message = "You need one available team member. Some of your members are already registered.";
    }
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && empty($error_message)) {
        try {
            ...
            if (!isset($_POST['teammate']) || empty($_POST['teammate'])) {
                throw new Exception("Please select one teammate.");
            }

            // Verify the selected teammate is valid and available
            $selected_teammate = array_filter($available_members, function($member) {
                return $member['id'] == $_POST['teammate'];
            });

            if (empty($selected_teammate)) {
                throw new Exception("Invalid teammate selection or teammate already registered.");
            }
                ...
                <div class="col-md-8">
                    <div class="card">
                        <div class="card-header">
                            <h2 class="text-center mb-0">Duo Registration</h2>
                        </div>
                        <div class="card-body">
                                    ...
                                    <p class="mb-0">
                                        ...
                                        <ul>
                                            <?php if (empty($team_members)): ?>
                                                <li>Add at least one member to your team</li>
                                                <li>Wait for members to accept their invitations</li>
                                            <?php else: ?>
                                                <li>Ensure you have one available member</li>
                            ...
                            <?php else: ?>
                                <form method="POST" id="registrationForm">
                                    ...
                                    <div class="mb-3">
                                        <label class="form-label">Select Your Teammate</label>
                                        <div class="team-members-grid">
                                            <?php foreach ($team_members as $member): ?>
                                                <div class="member-card <?php echo $member['is_registered'] ? 'unavailable' : ''; ?>">
                                                    <input type="radio" name="teammate" 
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const radioButtons = form.querySelectorAll('input[type="radio"]:not([disabled])');
    ...
    form.addEventListener('submit', function(e) {
        const selectedTeammate = form.querySelector('input[name="teammate"]:checked');
        if (!selectedTeammate) {
            e.preventDefault();
